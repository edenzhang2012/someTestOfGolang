...
func Zip(zipPath string, key []byte, paths ...string) error {
	...
	if key != nil {
		...
		writer := &cipher.StreamWriter{S: stream, W: encrypt}
		zipWriter = zip.NewWriter(writer)
		defer zipWriter.Close()
	} else {
		zipWriter = zip.NewWriter(encrypt)
		defer zipWriter.Close()
	}
...
func Unzip(zipath, dir string, passwd []byte) error {
	// Open zip file.
	...
	for _, file := range reader.File {
		if err := unzipFile(file, dir, passwd); err != nil {
			return err
		}
	}
	return nil
...
func unzipFile(file *zip.File, dir string, passwd []byte) error {
